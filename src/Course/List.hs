...
--
-- >>> ffilter even (1 :. 2 :. 3 :. 4 :. 5 :. Nil) ilter even (1 :. 2 :. 3 :. 4 :. 5 :. Nil)
-- [2,4]
--
...
-- (True :. False :. True :. Nil)
-- (a    :.          c    :. Nil)
filter f = foldRight (\a list -> if f a then a :. list else list) Nil
...
seqOptional ::
  List (Optional a) ->
  Optional (List a)
seqOptional = foldRight step last
  where
    last = Full Nil
    step optionalA optionalList =
      case (optionalA, optionalList) of
        (Full b, Full x) -> Full (b :. x)
        _ -> Empty
...
find ::
  ...
  List a ->
  Optional a
find f = foldRight step Empty
  where
    step a possibleMatch =
      case (f a, possibleMatch) of
        (True, _) -> Full a
        (_, Full x) -> Full x
        (False, Empty) -> Empty
